...
static const char * c4_debug_family_names[5] = { "nargin", "nargout", "A", "B",
  "z" };
...
static real_T c4_emlrt_marshallIn(SFc4_untitledInstanceStruct *chartInstance,
  const mxArray *c4_z, const char_T *c4_identifier);
...
static void sf_c4_untitled(SFc4_untitledInstanceStruct *chartInstance)
{
  ...
  c4_b_A = (real_T *)ssGetInputPortSignal(chartInstance->S, 0);
  _sfTime_ = (real_T)ssGetT(chartInstance->S);
  _SFD_CC_CALL(CHART_ENTER_SFUNCTION_TAG, 3U, chartInstance->c4_sfEvent);
  _SFD_DATA_RANGE_CHECK(*c4_b_A, 0U);
  _SFD_DATA_RANGE_CHECK(*c4_b_z, 1U);
  _SFD_DATA_RANGE_CHECK(*c4_b_B, 2U);
  chartInstance->c4_sfEvent = CALL_EVENT;
  _SFD_CC_CALL(CHART_ENTER_DURING_FUNCTION_TAG, 3U, chartInstance->c4_sfEvent);
  c4_hoistedGlobal = *c4_b_A;
  ...
  sf_debug_symbol_scope_add_eml_importable(&c4_nargout, 1U, c4_sf_marshallOut,
    c4_sf_marshallIn);
  sf_debug_symbol_scope_add_eml(&c4_A, 2U, c4_sf_marshallOut);
  sf_debug_symbol_scope_add_eml(&c4_B, 3U, c4_sf_marshallOut);
  sf_debug_symbol_scope_add_eml_importable(&c4_z, 4U, c4_sf_marshallOut,
    c4_sf_marshallIn);
  CV_EML_FCN(0, 0);
...
static const mxArray *sf_get_sim_state_info_c4_untitled(void)
{
  ...
  const char *infoEncStr[] = {
    "100 S1x2'type','srcId','name','auxInfo'{{M[1],M[6],T\"z\",},{M[8],M[0],T\"is_active_c4_untitled\",}}"
  };
    ...
    if (ssIsFirstInitCond(S) && fullDebuggerInitialization==1) {
      /* do this only if simulation is starting */
      {
...

        {
          ...
          _SFD_SET_DATA_VALUE_PTR(1U, c4_z);
...
{
  SFc4_untitledInstanceStruct *chartInstance;
  chartInstance = (SFc4_untitledInstanceStruct *)malloc(sizeof
